...
function getDirListAboveASize(fileSystem: Directory, sizeLimit: number) {
	const dirList = [];

	if (fileSystem.size >= sizeLimit) dirList.push(fileSystem);

	fileSystem.subDir.forEach(dir => {
		const subDirList = getDirListAboveASize(dir, sizeLimit);
...
function findSizeOfSmallestDirectoryToFreeUpSpace(input: string[]) {
	const fileSystem = parseFileSystem(input);
	const totalUsedSpace = fileSystem.size;

	const unusedSpace = TOTAL_DISK_SPACE - totalUsedSpace;
	const spaceToRemove = UNUSED_SPACE_NEEDED_FOR_UPDATE - unusedSpace;

	const dirList = getDirListAboveASize(fileSystem, spaceToRemove);
	const sizeList = dirList.map(dir => dir.size);
...
console.log(findSizeOfSmallestDirectoryToFreeUpSpace(splitInput));